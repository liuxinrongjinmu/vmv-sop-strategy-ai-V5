...
class OrchestratorAgent:
    ...
    async def process_message(
        ...
        elif intent == "generate_five_year_report":
            return await self._handle_five_year_report(message, session_info, uploaded_files)
        elif intent == "generate_three_year_report":
            return await self._handle_three_year_report(message, session_info, uploaded_files)
        elif intent == "generate_one_year_report":
            return await self._handle_one_year_report(message, session_info, uploaded_files)
    ...
    async def process_message_stream(
        ...
        """
        流式处理用户消息
        对general_chat类型使用LLM流式生成，逐token yield输出
        对report和stage_transition类型直接yield完整dict
        """
        if uploaded_files is None:
            uploaded_files = []
        if chat_history is None:
        ...
        intent = await self._detect_intent(message, current_stage)
        logger.info(f"流式意图检测: message='{message[:50]}...', intent={intent}, stage={current_stage}")

        if intent == "generate_report":
            result = await self._handle_report_generation(message, session_info, uploaded_files)
            yield result
        elif intent == "generate_five_year_report":
            result = await self._handle_five_year_report(message, session_info, uploaded_files)
            yield result
        elif intent == "generate_three_year_report":
            result = await self._handle_three_year_report(message, session_info, uploaded_files)
            yield result
        elif intent == "generate_one_year_report":
            result = await self._handle_one_year_report(message, session_info, uploaded_files)
            yield result
        elif intent == "stage_transition":
            result = await self._handle_stage_transition(current_stage)
            yield result
        else:
            async for event in self._handle_general_chat_stream(message, session_info, current_stage, uploaded_files, chat_history):
                yield event
    ...
    async def _detect_intent(self, message: str, current_stage: int) -> str:
        ...
        five_year_keywords = ["五年战略", "五年分析", "关键驱动因素", "驱动因素分析", "五年规划"]
        if any(kw in message_stripped for kw in five_year_keywords):
            return "generate_five_year_report"

        three_year_keywords = ["三年目标", "三年规划", "阶段性目标", "三年战略"]
        if any(kw in message_stripped for kw in three_year_keywords):
            return "generate_three_year_report"

        one_year_keywords = ["一年计划", "年度计划", "战略屋", "任务分解", "年度战略", "SOP"]
        if any(kw in message_stripped for kw in one_year_keywords):
            return "generate_one_year_report"
...
意图类别:
- REPORT: 用户在表达对赛道/行业的未来预判、预测、展望，希望生成十年战略分析报告
- FIVE_YEAR: 用户希望进行五年关键驱动因素分析
- THREE_YEAR: 用户希望设定三年阶段性目标
- ONE_YEAR: 用户希望进行一年任务分解与战略屋构建
- TRANSITION: 用户表示当前阶段的信息已完整，想进入下一阶段
- CHAT: 普通对话、提问、补充信息、讨论

只返回一个词: REPORT / FIVE_YEAR / THREE_YEAR / ONE_YEAR / TRANSITION / CHAT"""

        response = await llm_service.generate(prompt, temperature=0.1, max_tokens=10, provider=None)
        result = response.strip().upper()
        
        if "FIVE_YEAR" in result:
            return "generate_five_year_report"
        elif "THREE_YEAR" in result:
            return "generate_three_year_report"
        elif "ONE_YEAR" in result:
            return "generate_one_year_report"
    ...
    async def _handle_report_generation(self, prediction: str, session_info: Dict, uploaded_files: List[Dict] = None) -> Dict[str, Any]:
        """
        处理报告生成意图
        仅返回触发信号，实际报告由异步API生成（避免阻塞聊天请求 + 避免双重生成）
        """
        return {
            "type": "report",
            "content": "系统检测到您的预判，正在启动十年战略分析...\n\n请稍候，报告生成需要约1-2分钟。生成完成后将自动展示结果。",
            "stage": 4
        }

    async def _handle_five_year_report(self, prediction: str, session_info: Dict, uploaded_files: List[Dict] = None) -> Dict[str, Any]:
        """
        处理五年关键驱动因素分析意图
        """
        return {
            "type": "report",
            "content": "系统检测到您需要五年关键驱动因素分析，正在启动分析...\n\n请稍候，报告生成需要约1-2分钟。",
            "stage": 4,
            "report_type": "five_year"
        }

    async def _handle_three_year_report(self, prediction: str, session_info: Dict, uploaded_files: List[Dict] = None) -> Dict[str, Any]:
        """
        处理三年阶段性目标设定意图
        """
        return {
            "type": "report",
            "content": "系统检测到您需要三年阶段性目标设定，正在启动分析...\n\n请稍候，报告生成需要约1-2分钟。",
            "stage": 4,
            "report_type": "three_year"
        }

    async def _handle_one_year_report(self, prediction: str, session_info: Dict, uploaded_files: List[Dict] = None) -> Dict[str, Any]:
        """
        处理一年任务分解与战略屋意图
        """
        return {
            "type": "report",
            "content": "系统检测到您需要一年任务分解与战略屋，正在启动分析...\n\n请稍候，报告生成需要约1-2分钟。",
            "stage": 4,
...
5. 回复简洁有力，不要过于冗长"""
    ...
    async def _handle_general_chat_stream(
        self,
        message: str,
        session_info: Dict,
        current_stage: int,
        ...
        """
        流式处理普通对话
        逐token yield {"type": "text", "content": "token"}，最后yield meta事件
        """
        context = self._build_context(session_info, current_stage)
...
5. 回复简洁有力，不要过于冗长"""
        ...
        async for token in llm_service.generate_stream(prompt, temperature=0.6):
            yield {"type": "text", "content": token}

        yield {"type": "meta", "stage": current_stage, "sources": []}
